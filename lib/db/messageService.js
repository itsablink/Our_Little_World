// Database service abstraction layer for text-based Message Memories.
//
// In production (Vercel), metadata and message text are stored in persistent
// database storage (Postgres, Supabase, Vercel KV) via process.env.DATABASE_URL.
//
...
export async function updateMessageInDb(id, patch) {
  if (process.env.DATABASE_URL) {
    try {
      ...
      console.error("[MessageService] Database update error:", err);
      throw new Error("Failed to update message in persistent database");
    }
  }
